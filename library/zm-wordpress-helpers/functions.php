...
function zm_base_get_the_term_list( $post_id=null, $taxonomy=null, $before = '', $sep = ', ', $after = '' ) {
    ...
    if ( $terms && !is_wp_error( $terms ) ) {
        foreach ( $terms as $term ) {
                ...
                $term_links[] = '<a href="' . $my_link . '" title="'.$title.'" rel="'.$term->taxonomy . '_' . $term->slug.'" class="zm-base-'. $taxonomy.'-'.$term->slug .' ' . $extra_class. '" data-content="'. $description .'">' . $term->name . '</a>';            
        }
...
function zm_base_list_terms( $taxonomy ) {
    ...
    foreach( $terms as $term ) {
        ...
        // First
        if ( $i == 0 )
            $html .= '<li class="zm-base-title ' . $term->taxonomy . '">' . $label .'</li>';    
...
//        exit( "Opps..." . $terms->get_error_message() . "..dog, cmon, fix it!" );
    ...
    <select name="<?php echo $taxonomy; ?><?php echo $multiple_extra;?>" <?php echo $multiple; ?> <?php echo $extra_data; ?> class="<?php echo $extra_class; ?>" id="" <?php echo $multiple; ?>>
        <?php // First choice ?>
        <option value="">-- Choose a <?php echo $label; ?> --</option>              
...
//        exit( "Opps..." . $terms->get_error_message() . "..dog, cmon, fix it!" );
    ...
    <?php foreach( $terms as $term ) : ?>
        ...
        <input 
            ...
            <?php echo $selected; ?> />
        <?php echo $term->name; ?> 
        <span class="zm-base-count"><?php echo $term->count; ?></span>
        </label>
    <?php endforeach; ?>
    </fieldset>
    <?php endif; ?>
